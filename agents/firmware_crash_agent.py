...
def analyze_with_llm(state: dict) -> str:
    ...
    base_prompt = f"""Bạn là firmware_crash_agent. Nhiệm vụ của bạn là thu thập thông tin thô bằng tools dựa trên yêu cầu từ alert.
...
"""
    ...
    system_prompt = get_system_prompt("event_log_agent")
    ...
    final_note = run_react_loop(
        agent_name="firmware_crash_agent",
    ...
    state.setdefault("blackboard", {})["firmware_crash_agent"] = final_note
    print(f"[firmware_crash_agent] Diagnosis generated (length={len(final_note)})")
    return final_note
