...
TOOL_REGISTRY = {
    ...
    "get_switch_dhcp_server_policy": lambda net_id, serial: meraki.get_switch_dhcp_server_policy(net_id),
    "get_switch_routing_interfaces": lambda net_id, serial: meraki.get_switch_routing_interfaces(serial),
    ...
    "start_packet_capture": lambda net_id, serial: meraki.start_packet_capture(serial),
    "get_network_vlans": lambda net_id, serial: meraki.get_network_vlans(net_id),
    "get_dai_trusted_servers": lambda net_id, serial: meraki.get_dai_trusted_servers(net_id),
    "get_switch_port_history": lambda net_id, serial: meraki.get_switch_port_history(serial),
    "clear_switch_port_counters": lambda net_id, serial: meraki.clear_switch_port_counters(serial),
    "get_cable_test_history": lambda net_id, serial: meraki.get_cable_test_history(serial),
    "get_switch_static_routes": lambda net_id, serial: meraki.get_switch_static_routes(serial),
    "set_switch_port_poe": lambda net_id, serial: meraki.set_switch_port_poe(serial),
    "cycle_switch_ports": lambda net_id, serial: meraki.cycle_switch_ports(serial, [1]),
    "get_network_switch_stacks": lambda net_id, serial: meraki.get_network_switch_stacks(net_id),
    "get_network_switch_stp": lambda net_id, serial: meraki.get_network_switch_stp(net_id),
    "get_network_topology": lambda net_id, serial: meraki.get_network_topology(net_id)
}
...
def analyze_with_llm(state: dict) -> str:
    ...
    base_prompt = f"""Bạn là switch_port_agent. Nhiệm vụ của bạn là thu thập thông tin thô bằng tools dựa trên yêu cầu từ alert.
...
"""
    ...
    system_prompt = get_system_prompt("switch_port_agent")
    ...
    state.setdefault("blackboard", {})["switch_port_agent"] = final_note
    print(f"[switch_port_agent] Diagnosis generated (length={len(final_note)})")
    return final_note
