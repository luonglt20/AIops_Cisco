...
def analyze_with_llm(state: dict) -> str:
    ...
    # Chỉ định mapping các tool cho agent này. 
    # Các tool sẽ được fill filter theo allowed_tools từ Coordinator.
    TOOL_REGISTRY = {
        ...
        "get_appliance_uplinks": lambda net_id, serial: meraki.get_appliance_uplinks(org_id),
        ...
        "get_uplink_loss_latency": lambda net_id, serial: meraki.get_uplink_loss_latency(org_id),
    ...
    base_prompt = f"""Bạn là client_experience_agent. Nhiệm vụ của bạn là thu thập thông tin thô bằng tools dựa trên yêu cầu từ alert.
...
"""
    ...
    system_prompt = get_system_prompt("client_experience_agent")
    ...
    final_note = run_react_loop(
        agent_name="client_experience_agent",
    ...
    state.setdefault("blackboard", {})["client_experience_agent"] = final_note
    print(f"[client_experience_agent] Diagnosis generated (length={len(final_note)})")
    return final_note
