...
TOOL_REGISTRY = {
    ...
    "get_wireless_failed_connections": lambda net_id, serial: meraki.get_wireless_failed_connections(net_id),
    "get_network_ssids": lambda net_id, serial: meraki.get_network_ssids(net_id),
    "get_wireless_rf_profiles": lambda net_id, serial: meraki.get_wireless_rf_profiles(net_id),
    "reboot_individual_ssid": lambda net_id, serial: meraki.reboot_individual_ssid(net_id),
    "get_cochannel_interference": lambda net_id, serial: meraki.get_cochannel_interference(net_id),
    "get_rf_profile_assignments": lambda net_id, serial: meraki.get_rf_profile_assignments(net_id),
    "get_wireless_mesh_statuses": lambda net_id, serial: meraki.get_wireless_mesh_statuses(net_id)
}
...
def analyze_with_llm(state: dict) -> str:
    ...
    base_prompt = f"""Bạn là rf_wireless_agent. Nhiệm vụ của bạn là thu thập thông tin thô bằng tools dựa trên yêu cầu từ alert.
...
"""
    ...
    system_prompt = get_system_prompt("rf_wireless_agent")
    ...
    state.setdefault("blackboard", {})["rf_wireless_agent"] = final_note
    print(f"[rf_wireless_agent] Diagnosis generated (length={len(final_note)})")
    return final_note
