...
TOOL_REGISTRY = {
    ...
    "get_sensor_reading_history": lambda net_id, serial: meraki.get_sensor_reading_history(net_id),
    "get_camera_heatmap_zones": lambda net_id, serial: meraki.get_camera_heatmap_zones(net_id),
    "get_ble_settings": lambda net_id, serial: meraki.get_ble_settings(net_id)
}
...
def analyze_with_llm(state: dict) -> str:
    ...
    base_prompt = f"""Bạn là sensor_iot_agent. Nhiệm vụ của bạn là thu thập thông tin thô bằng tools dựa trên yêu cầu từ alert.
...
"""
    ...
    system_prompt = get_system_prompt("sensor_iot_agent")
    ...
    state.setdefault("blackboard", {})["sensor_iot_agent"] = final_note
    print(f"[sensor_iot_agent] Diagnosis generated (length={len(final_note)})")
    return final_note
