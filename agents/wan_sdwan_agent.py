...
TOOL_REGISTRY = {
    ...
    "get_appliance_performance": lambda net_id, serial: meraki.get_appliance_performance(net_id),
    "get_appliance_security_events": lambda net_id, serial: meraki.get_appliance_security_events(net_id),
    ...
    "run_traceroute_test": lambda net_id, serial: meraki.run_traceroute_test(serial),
    "get_security_intrusion": lambda net_id, serial: meraki.get_security_intrusion(net_id),
    "get_amp_malware_settings": lambda net_id, serial: meraki.get_amp_malware_settings(net_id),
    "get_thousandeyes_media_perf": lambda net_id, serial: meraki.get_thousandeyes_media_perf(net_id),
    "run_multi_target_mtr": lambda net_id, serial: meraki.run_multi_target_mtr(serial),
    "get_appliance_ports_config": lambda net_id, serial: meraki.get_appliance_ports_config(net_id),
    "get_sdwan_traffic_shaping": lambda net_id, serial: meraki.get_sdwan_traffic_shaping(net_id),
    "get_third_party_vpn_peers": lambda net_id, serial: meraki.get_third_party_vpn_peers(net_id),
    "get_network_vpn_status": lambda net_id, serial: meraki.get_network_vpn_status(net_id),
    "get_network_l7_firewall_rules": lambda net_id, serial: meraki.get_network_l7_firewall_rules(net_id)
}
...
def analyze_with_llm(state: dict) -> str:
    ...
    base_prompt = f"""Bạn là wan_sdwan_agent. Nhiệm vụ của bạn là thu thập thông tin thô bằng tools dựa trên yêu cầu từ alert.
...
"""
    ...
    system_prompt = get_system_prompt("wan_sdwan_agent")
    ...
    state.setdefault("blackboard", {})["wan_sdwan_agent"] = final_note
    print(f"[wan_sdwan_agent] Diagnosis generated (length={len(final_note)})")
    return final_note
